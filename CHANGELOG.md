...
- Added an internal fast path that adopts a caller-prefetched live chat config instead of issuing a duplicate `getChatConfig` request. The payload is treated as untrusted and is adopted only when the caller's attestation, the payload's own identity, and the payload shape all match the SDK instance; any rejection or adoption failure falls back to the normal network fetch, and both outcomes are recorded as telemetry. This is an unsupported internal optimization, not public API: it is deliberately absent from the exported `GetLiveChatConfigOptionalParams` type, and it is sound only while the config stays inside the caller's own realm.
